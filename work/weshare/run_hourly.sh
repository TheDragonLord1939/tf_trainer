...
function monitor_eval()
{
  ...
  if [[ ${BASE_ONLINE_DICT_LOCAL_DIR} =~ /$ ]]; then
      local_dict_dir=${BASE_ONLINE_DICT_LOCAL_DIR}${model_date}/${model_hour}
  else
      local_dict_dir=${BASE_ONLINE_DICT_LOCAL_DIR}/${model_date}/${model_hour}
  fi
  ...
  base_key="SPRS.Model.hourly.${ONLINE_DICT_NAME}.${key_type}"

  NOTICE_LOG "Starting importing monitor of ${model_date}_${model_hour}" ${log_path}
  cat ${save_path} | python3 ${RUN_DIR}/scripts/import.py "${base_key}" "${model_date}_${model_hour}0000" >> ${log_path} 2>&1
  NOTICE_LOG "Finished importing monitor of ${model_date}_${model_hour}, ret=${is_model_valid}" ${log_path}
}
...
function push_model()
{
    ...
    date=${version: 0: 8}
    hour=${version: 8: 10}
    rm_date=`date -d "${date} ${hour} -2days" +"%Y%m%d%H"`
    if [[ ${BASE_ONLINE_DICT_DIR} =~ /$ ]]; then
...
function collect_model()
{
    ...
    feature_index_path="${ssd_dir}/${date}/${hour}/feature_index/"
    ...
    cp -r ${WORK_DIR}/${date}/${hour}/summaries/${MODEL}_${TASK_NAME}/export/best/*/* ${local_dict_dir}
    if [ ! -f "${local_dict_dir}/saved_model.pb" ]; then
    ...
    if [ "${DNN_WITH_HASH}" == "True" ]; then
        feature_index="feature_index_v1"
    else
        feature_index="feature_index_v2"
    fi

    cat ${feature_index_path}/part-* | \
        python3 ${RUN_DIR}/scripts/${FEATURE_INDEX_SCRIPT} ${feature_map_file} ${feature_index} "${ONLINE_BLACK_FEATURES}" "${IS_SHARE_EMB}" \
        2>>${log_path} 1>${dict_txt}

    if [ $? -ne 0 ]; then
        ERROR_LOG "Running ${FEATURE_INDEX_SCRIPT} for ${feature_index} failed" ${log_path}
    fi
...
function cdn_download()
{
    ...
    if [[ ${BASE_CDN_DATA_DIR} =~ /$ ]]; then
        python3 ${RUN_DIR}/scripts/download.py ${BASE_CDN_DATA_DIR}${date}/${hour}  >> ${log_path} 2>&1
    else
        python3 ${RUN_DIR}/scripts/download.py ${BASE_CDN_DATA_DIR}/${date}/${hour} >> ${log_path} 2>&1
    fi
...
function huawei_download()
{
    ...
    if [[ ${BASE_CDN_DATA_DIR} =~ /$ ]]; then
        data_dir=${BASE_CDN_DATA_DIR}${date}/${hour}
    else
        data_dir=${BASE_CDN_DATA_DIR}/${date}/${hour}
    fi
...
function prepare_train()
{
    ...
    if [ "${DNN_WITH_HASH}" == "True" ]; then
        feature_list="feature_list_v1"
    else
        feature_list="feature_list_v2"
    fi

    cat ${ssd_dir}/${date}/${hour}/feature_index/part-* | \
        python3 ${RUN_DIR}/scripts/${FEATURE_INDEX_SCRIPT} ${feature_map_file} ${feature_list} "${ONLINE_BLACK_FEATURES}" "${IS_SHARE_EMB}" \
        2>>${log_path} 1>${feature_lst_path}

    if [ $? -ne 0 ]; then
        ERROR_LOG "Running ${FEATURE_INDEX_SCRIPT} for ${feature_list} failed" ${log_path}
        exit 0
    fi
    ...
    python3 ${RUN_DIR}/scripts/gen_cfg.py ${ssd_dir}/${date}/${hour} ${work_dir} 2>>${log_path}
...
function eval()
{
    ...
    model_dir="${WORK_DIR}/${model_date}/${model_hour}/summaries/${MODEL}_${TASK_NAME}"
...
function feature_influence()
{
    ...
    eval ${work_dir} ${log_path} ${model_date} ${model_hour}
    ...
    do
      eval ${work_dir} ${log_path} ${model_date} ${model_hour} $feat
...
function clean_data()
{
    ...
    if [ "${WORK_ON_SSD_MHD}" == "ssd" ]; then
        ...
        rm -rf ${ssd_dir}/${rm_date}/${hour}
        NOTICE_LOG "Delete data=${ssd_dir}/${rm_date}/${hour}" ${log_path}

        # clean local_online_dict
        rm_date=`date -d "${date} -5days" +"%Y%m%d"`
        rm -rf ${BASE_ONLINE_DICT_LOCAL_DIR}/${rm_date}${hour}
        NOTICE_LOG "Delete online_dict=${BASE_ONLINE_DICT_LOCAL_DIR}/${rm_date}${hour}" ${log_path}

        # clean local_work_dir
        rm_date=`date -d "${date} -5days" +"%Y%m%d"`
        rm -rf ${WORK_DIR}/${rm_date}/${hour}
        NOTICE_LOG "Delete work_dir=${WORK_DIR}/${rm_date}/${hour}" ${log_path}

    else
        # clean train data
        rm_date=`date -d "${date} -7days" +"%Y%m%d"`
        rm -rf ${mhd_dir}/${rm_date}/${hour}
        NOTICE_LOG "Delete data=${mhd_dir}/${rm_date}/${hour}" ${log_path}
...
function send_mail()
{
    ...
    subject="${MAIL_SUBJECT} ${date}_${hour}"
...
function pipeline2()
{
    ...
    # check data
    if [ ! -d ${ssd_save_dir}/${date}/${hour} ]; then
        ERROR_LOG "Data=${ssd_save_dir}/${date}/${hour} does not exist" ${log_path}
        exit 0
    fi

    # prepara_train
    prepare_train ${date} ${hour} ${ssd_save_dir} ${work_dir} ${log_path}
    ...
    if [ "${USE_INCREMENT_MODE}" == "True" ]; then
        backup_increment_model ${work_dir} ${log_path} ${date}${hour}
    fi

    # collect_model
    collect_model ${ssd_save_dir} ${log_path} ${date} ${hour}

    if [ "${DO_EVAL}" == 'True' ]; then
      if [ "${IS_POSTERIOR_VALID}" == "False" ]; then
          # eval
          eval ${work_dir} ${log_path} ${date} ${hour}

          # monitor
          monitor_eval ${date} ${hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
          ...
          # monitor
          monitor_eval ${date} ${hour} ${log_path} ${work_dir} ${TRN_VALID_FLAG} "train"

          # eval
          eval ${work_dir} ${log_path} ${date} ${last_hour}

          # monitor
          monitor_eval ${date} ${last_hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
      ...
      if [ "${IS_POSTERIOR_VALID}" == "False" ]; then
          # predict
          predict ${work_dir} ${log_path} ${TST_VALID_FLAG} ${date} ${hour}

          # monitor
          monitor ${date} ${hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
          ...
          predict ${work_dir} ${log_path} ${TRN_VALID_FLAG} ${date} ${hour}

          # monitor current model with train data
          monitor ${date} ${hour} ${log_path} ${work_dir} ${TRN_VALID_FLAG} "train"

          # predict with last datepart model on current train data
          predict ${work_dir} ${log_path} ${TST_VALID_FLAG} ${last_date} ${last_hour}

          # monitor last model with train data
          monitor ${last_date} ${last_hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
    ...
    # push_online
    push_model ${date}${hour} ${log_path}

    # send mail
    send_mail ${date} ${hour} ${work_dir} ${log_path}

    if [ ${PIPELINE_CLEAN_DATA} == "True" ]; then
        # clean data
        clean_data ${date} ${hour} ${ssd_save_dir} ${mhd_save_dir}
    fi
}

if [ $# -lt 3 ]; then
    echo "Usage: $0 [conf_file] [task_type] [date] ${hour}"
    echo "[date] is optional, if missing, set it to yesterday"
    echo "[hour] is optional, if missing, set it to last hour"
...
last_date=`date -d "${date} ${hour} -1hours" +"%Y%m%d"`
last_hour=`date -d "${date} ${hour} -1hours" +"%H"`

work_dir="${WORK_DIR}/${date}/${hour}"
if [ ! -d ${work_dir} ]; then
    mkdir -p ${work_dir}
fi
...
if [ "${task_type}" == "cdn_index" ]; then
    ...
    python3 index.py ${BASE_CDN_DATA_DIR}/${date}/${hour}
    NOTICE_LOG "Finished generating index" ${log_path}

elif [ "${task_type}" == "download" ]; then
    #cdn_download ${date} ${log_path}
    if [ ! -f ${mhd_save_dir}${date}/${hour/}_SUCCESS ]; then
        huawei_download ${date} ${log_path} ${mhd_save_dir}${date} ${hour}
    else
        NOTICE_LOG "${mhd_save_dir}${date}/${hour} exists, skip downloading" ${log_path}
    fi
    if [ "${mhd_save_dir}" != "${ssd_save_dir}" ]; then
        NOTICE_LOG "Starting copying data from mhd to ssd" ${log_path}
        cp -r ${mhd_save_dir}/${date}/${hour} ${ssd_save_dir}/${date}
    ...
    if [ "${IS_POSTERIOR_VALID}" == "False" ]; then
        ...
        eval ${work_dir} ${log_path} ${date} ${hour}
        eval ${work_dir} ${log_path} ${last_date} ${last_hour}
    fi
    ...
    if [ "${IS_POSTERIOR_VALID}" == "False" ]; then
        ...
        predict ${work_dir} ${log_path} ${TRN_VALID_FLAG} ${date} ${hour}
        predict ${work_dir} ${log_path} ${TST_VALID_FLAG} ${last_date} ${last_date}
    fi
    ...
    if [ "${IS_POSTERIOR_VALID}" == "False" ]; then
        monitor_eval ${last_date} ${last_hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
    else
        monitor_eval ${date} ${hour} ${log_path} ${work_dir} ${TRN_VALID_FLAG} "train"
        monitor_eval ${last_date} ${last_hour} ${log_path} ${work_dir} ${TST_VALID_FLAG} "test"
    fi
    ...
    pipeline2 ${date} ${hour} ${ssd_save_dir} ${work_dir} ${log_path}
