...
function train_model()
{
    ...
    CUDA_VISIBLE_DEVICES=${CUDA_DEVICE_ID} python3 ${train_py_route} \
        ...
        --weight_neg_value ${WEIGHT_NEG_VALUE} \
...
function push_model_s3()
{
    ...
    /usr/local/bin/aws s3 sync $local_dict_dir/ $online_dict_dir/
    
	NOTICE_LOG "Finished pushing model to aws" ${log_path}

    rm_date=`date -d "${version} -2days" +"%Y%m%d"`
    if [[ ${BASE_ONLINE_DICT_DIR} =~ /$ ]]; then
        rm_online_model="${BASE_ONLINE_DICT_DIR}${ONLINE_DICT_NAME}/${rm_date}"
    else
        rm_online_model="${BASE_ONLINE_DICT_DIR}/${ONLINE_DICT_NAME}/${rm_date}"
    fi
	NOTICE_LOG "rm_online_mode=$rm_online_model" ${log_path}
    /usr/local/bin/aws s3 rm  ${rm_online_model} --recursive >> ${log_path}
...
function prepare_train()
{
    ...
    cat ${ssd_dir}/${date}/feature_index/* | \
...
if [ "${task_type}" == "cdn_index" ]; then
	...
	while true
    do
        if [ ! -f "${mhd_save_dir}/${date}/_SUCCESS" ]; then
            NOTICE_LOG "${mhd_save_dir}/${date}/_SUCCESS is not ready yet" ${log_path}
            sleep 300
        else
            break
    fi
    ...
    pipeline2 ${date} ${last_date} ${ssd_save_dir} ${work_dir} ${log_path}
elif [ "${task_type}" == "pipeline3" ]; then
    stop_date=`date -d"-1days" +%Y%m%d`
    if [ $# -eq 4 ]; then
        stop_date=$4
    fi
    
    while [ $date -lt $stop_date ]
    do
        echo $date" "$last_date" "$stop_date 
        #huawei_download ${date} ${log_path} ${mhd_save_dir}
        pipeline2 ${date} ${last_date} ${ssd_save_dir} ${work_dir} ${log_path}
