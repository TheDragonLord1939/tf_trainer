...
class MailSender:
    def __init__(self):
        self.__fromaddr = 'husong_wangyi@163.com'
        self.__username = 'husong_wangyi'
        self.__password = 'hs982121'
    ...
    def send(self, recevier_list, subject, text_message, attach_file_list):
        ...
        try:
            self.__server = SMTP_SSL('smtp.163.com', 25)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        ...
        Notify("【精排模型】").send_mail(sys.argv[2], sys.argv[3], sys.argv[1])
        # sendmail(sys.argv[1], sys.argv[2], sys.argv[3])
    else:
        Notify("【精排模型】").send_mail(sys.argv[2], sys.argv[3], sys.argv[1], sys.argv[4])
