...
def PrAuc(labels, preds):
    from sklearn import metrics
    precision, recall, _ = metrics.precision_recall_curve(labels, preds)

    return metrics.auc(recall, precision)  # 梯形块分割，建议使用
...
def main(valid_flag):
    ...
    prauc = PrAuc(labels, preds)
    if len(groups) > 100:
        gauc = Gauc(labels, preds, groups)
    else:
        gauc = 0
    ctr, loss, norm_loss, calibration = EvalRec(labels, preds)
    print('[NOTICE] %s ctr=%f loss=%.10f norm_loss=%.10f calibration=%f auc=%f gauc=%f prauc=%f' % (
        valid_flag, ctr, loss, norm_loss, calibration, auc, gauc, prauc
    ))
