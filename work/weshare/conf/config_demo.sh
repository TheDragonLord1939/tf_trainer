...
BASE_CDN_DATA_DIR="obs://sprs-data-sg/sharezone_cdn/xxx/"

# 线上模型在obs上存放的目录
BASE_ONLINE_DICT_DIR="obs://sprs-data-sg/sharezone_model_server/online_dict/deep_models"

# 线上模型在本地存放的目录
BASE_ONLINE_DICT_LOCAL_DIR="/tf/tensorflow/xxx/online_dict/"

# 执行run.sh相关任务的工作目录, 用于存放模型、日志等
WORK_DIR="/tf/tensorflow/xxx/"

# run.sh所在的相对目录 - 以tf_trainer为根目录
RUN_DIR="./work/weshare/"

# new features embedding conf [field_name size]
NEW_FEATURE_PATH="/data/zhaoyq/script/dict/new_feature.txt"

OUT_EMBEDDING_PATH="/data/zhaoyq/script/dict/new_emb/model.ckpt-5076"
WILL_TRANSFER_EMBEDDING_PATH="/data/zhaoyq/script/increment_model/model.ckpt-5076"

# 增量模型所在的本地目录, 若模型非增量设置为空即可
INCREMENT_MODEL_DIR="/tf/tensorflow/increment_model"

# 增量模型云端备份目录, 设置为空时不备份到云端
INCREMENT_MODEL_CLOUD_DIR="obs://sprs-data-sg/NeverDelete/weianjun/increment_model"
...
#---------- 模型更新校验参数配置 ----------#
VER_THRESH_AUC=0.68                     # 模型离线auc最低值
...
# 模型最低size, 单位为MB
VER_THRESH_MODEL_SIZE=180
#------------------------------------------#



#---------- 模型更新邮件通知配置 ----------#
MAIL_ADDRESS="xxx@ushareit.com,xxx@ushareit.com"
MAIL_SUBJECT="DNN Model Updating"
#------------------------------------------#
