...
BASE_CDN_DATA_DIR="obs://sprs-data-sg/sharezone_cdn/weianjun/ugc_like"

# 线上模型在obs上存放的目录
BASE_ONLINE_DICT_DIR="obs://sprs-data-sg/sharezone_model_server/online_dict/deep_models"

# 线上模型在本地存放的目录
BASE_ONLINE_DICT_LOCAL_DIR="/data2/weianjun/online_dict/ugc_like/"

# 执行run.sh相关任务的工作目录, 用于存放模型、日志等
WORK_DIR="/data2/weianjun/online_work_dir/ugc_like/"
...
ONLINE_DICT_NAME="id_rpush_dnn_model"

# 在线预测时模型不需要的特征列表, 多个特征用逗号分隔
ONLINE_BLACK_FEATURES=""

# 模型所用特征列表, 必须放到./data/目录
FEATURE_MAP_NAME="ugc_like_v2.csv"
...
VER_THRESH_CALIBRATION_UPPER=100        # 模型离线calibration上限
...
#---------- 模型更新邮件通知配置 ----------#
MAIL_ADDRESS="weiaj@ushareit.com,sibb@ushareit.com"
MAIL_SUBJECT="UGC Like DNN Model Updating"
#------------------------------------------#
